...
export class Experience {
  ...
  animateIn() {
    ...
    gsap.fromTo(
      this._torus.scale, { x: 0.8, y: 0.8, z: 0.8 },
    ...
    gsap.fromTo(
      this._sphere.scale, { x: 0.8, y: 0.8, z: 0.8 },
    ...
    gsap.fromTo(
      this._cone.scale, { x: 0.8, y: 0.8, z: 0.8 },
      { x: 1.35, y: 1.35, z: 1.35 }
    );
    gsap.fromTo(
    ...
    gsap.fromTo(
      this._cone.rotation, { x: -0.2, z: 0.0 },
      { x: -0.3, z: 0.7 }
    );
  }
  ...
  bind() {
    ...
    document.getElementById("pink-button")!.addEventListener("mouseleave", () => {
  ...
  loop() {
    ...
    if (delta > this.fpsInterval) {
      // this.scene.update()
      this.camera.position.x += this.mouse.x * (this.container.clientWidth * 0.02) - this.camera.position.x * 0.03;
      this.camera.position.y +=
        -(this.mouse.y * (this.container.clientHeight * 0.02)) - this.camera.position.y * 0.03;
